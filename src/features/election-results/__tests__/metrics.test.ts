...
import { mergeComparisonLocalities, sortLocalityStatistics } from "../sort-locality-statistics";
...
describe("election result metrics", () => {
  ...
  it("filters mapped locality rows and sorts them by the selected list rank", () => {
    ...
    const filtered = buildLocalityStatistics(
      [first, second, hidden],
      "LIKUD",
      { shareMin: 30 },
      text,
    ...
    expect(
      sortLocalityStatistics(
        buildLocalityStatistics([first, second], "LIKUD", {}, text),
        "rank",
        "asc",
        text,
      ).map((item) => item.locality.localityId),
    ).toEqual([2, 1]);
  });
  ...
  it("unions A-only and B-only localities for comparison search and table rows", () => {
    ...
    const all = mergeComparisonLocalities([first], [second]);
    const rows = buildLocalityStatistics([first], "LIKUD", {}, text, {
      localities: [second],
      partyId: "LIKUD",
    });
